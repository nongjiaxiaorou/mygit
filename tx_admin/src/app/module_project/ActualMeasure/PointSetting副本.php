<?php
	...
	// function deawPointPic($measureId,$pointStatus,$small_height,$small_width,$picSrcInit) {
	...
	// 					$savePointX = floatval($parXcoordinate )+ floatval($Xcoordinate*$small_width/340) ;// 点的坐标
	// 					$savePointY = floatval($parYcoordinate) + floatval($Ycoordinate*$small_width/340);
	// 					$PointX = floatval($savePointX*$imageWidth/342);
	// 					$PointY = floatval($savePointY*$imageHeight/242);
	// 					if ($num == 'k15') {
	// 						$data['computeX'] = floatval($parXcoordinate ) . '+' . $Xcoordinate . '*' . $small_width . '/340';
	// 						$data['computeY'] = floatval($parYcoordinate ) . '+' . $Ycoordinate . '*' . $small_width . '/340';
	// 					}
	...
	switch($flag){
		...
		case 'savePoint':
			...
			// deawPointPic ($measureId,$pointStatus,$small_height,$small_width,$picSrcInit);
			// 删除旧图纸
			$sql = "SELECT * from `tb_inspectaccept_measure` WHERE `id`= '$measureId'";
			$result = $conn->query($sql);
			if($result)	{
				...
				if ($pointStatus == '初测') {
					$data['img'] = $row['primaryMeasurePIC'];
					$test = delFile('../../../images/app_pic/inspectPic/upload/'.$row['primaryMeasurePIC']);
					$data['test'] = $test;
				} else {
					$data['img'] = $row['measurePointPic'];
					delFile('../../../images/app_pic/inspectPic/upload/'.$row['measurePointPic']);
				}
			}else {
				$data['code'] = 0;				
			}
