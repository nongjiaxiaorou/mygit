            ...
            <el-form :model="form" ref="dialogEdit" label-width="100px" :rules="formrules">
				<div class="flex">
					<el-form-item label="工程类别" class="flex-1 px-2" prop="projectType">
					    <el-select v-model="form.projectType" placeholder="请选择工程类别">
					...
					<el-form-item label="实测类型" class="flex-1 px-2" prop="measurementType">
					    <el-select v-model="form.measurementType" placeholder="请选择实测类型">
                ...
                <div class="flex">
					<el-form-item label="检查项目" prop="inspectionItem" class="flex-1 px-2">
					    <el-input v-model="form.inspectionItem" placeholder="例如: 抹灰工程"></el-input>
					</el-form-item>
                    <el-form-item label="检查内容" prop="inspectionContent" class="flex-1 px-2">
                        <el-input v-model="form.inspectionContent" placeholder="请输入检查内容"></el-input>
						<span class="Warning">{{tip}}</span>
                    </el-form-item>
                </div>
                <div class="flex">
                    <el-form-item label="最小标准值" prop="miniStandardValue" class="flex-1 px-2">
                        <el-input v-model="form.miniStandardValue" placeholder="例如: 1.0"></el-input>
                    </el-form-item>
                    <el-form-item label="最大标准值" prop="maxStandardValue" class="flex-1 px-2">
                        <el-input v-model="form.maxStandardValue" placeholder="例如: 2.0"></el-input>
                    </el-form-item>
                </div>
				<div class="flex">
...
<script>
	export default {
		name: 'EditStandardItem',
		props: ['dialogEdit','form','selectValue1'],
		data() {
			return {
				// formDate: {
				// 	projectType: '',
		...
		methods: {
			...
			dialogFormAdd() {
				var that = this
				var fd = new FormData();
				fd.append('flag', 'updateInspectionItem');
				fd.append('id', this.form.id);
				fd.append('projectType', this.form.projectType);
				fd.append('inspectionItem', this.form.inspectionItem);
				fd.append('inspectionContent', this.form.inspectionContent);
				fd.append('number', this.form.number);
				fd.append('miniStandardValue', this.form.miniStandardValue);
				fd.append('maxStandardValue', this.form.maxStandardValue);
				fd.append('measurementType', this.form.measurementType);
				fd.append('tableType', this.selectValue1)
				// console.log(this.$refs.dialogEdit.validate())
				...
				this.$refs.dialogEdit.validate((valid) => {
					if (valid) {
						...
						this.$axios.post(that.$adminUrl+`/system/measureStandard.php`, fd).then((res) => {
							...
							if (res.data.code) {
								setTimeout(() => {
									this.$emit('handleUpdateInspectionItemChild'); //初始化webscoket刷新视图
