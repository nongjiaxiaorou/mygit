<template>
	<el-dialog title="分包单位导入" :visible.sync="dialogImportUnit.show" :modal-append-to-body="false" width="40%" class="dialog"
	 @close="clearFileFunc">
...
<script>
	export default {
		...
		mounted() {
			this.ExcelFile = this.$adminUrl + `/excel_template/广东腾越分包商Excel表.xlsx`
		...
		methods: {
			...
			//导入按钮
			upload() {
				this.dialogImportUnit.show = false;
				this.fileList = ""
			...
			httpRequest(param) {
				...
				fd.append('flag', 'saveSubcontractor')
				...
				this.$axios.post(this.$adminUrl + `/PHPExcel/ExcelUpload.php`, fd, config).then(res => {
					setTimeout(() => {
						this.$emit('handleImportUnitChild'); //初始化webscoket刷新视图
						this.$message.success('分包商新增成功！');
					}, 500);
					// if(res.code===0){
