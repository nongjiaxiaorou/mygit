<template>
	<el-dialog title="Excel表导入测点数据" :visible.sync="dialogImport.show" :modal-append-to-body="false" width="40%" class="dialog"
	 @close="clearFileFunc">
...
<script>
	export default {
		...
		methods: {
			...
			httpRequest(param) {
                // console.log(param)
                var itemInspectInformation = sessionStorage.getItem('itemInspectInformation')
				itemInspectInformation = JSON.parse(itemInspectInformation)
				let fileObj = param.file // 相当于input里取得的files
				let fd = new FormData() // FormData 对象
				fd.append('file', fileObj) // 文件对象
                fd.append('flag', 'MeasuringPoint')
                fd.append('id', itemInspectInformation.id)
