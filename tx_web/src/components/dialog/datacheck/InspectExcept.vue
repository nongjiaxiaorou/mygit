    ...
    <div class="AddPro">
        <el-dialog title="筛选维度" :modal-append-to-body="false" customClass="customClass" :visible.sync="dialogExcept.show">
            <el-form ref="dialogExcept" label-width="100px">
				...
				<div class="flex">
					<el-form-item label="分部工程：" class="">
						<el-select v-model="itemValue" clearable placeholder="请选择" class="select-style">
						    <el-option
    ...
    methods: {
        ...
        dialogFormAdd() {
			...
			this.$axios.post(that.$adminUrl+`/datacheck/ItemInspect/ItemInspect.php`, fd).then((res) => {
				...
				if (res.data.code) {  
				    setTimeout(() => {
				        this.$emit('handleSelectChild',res.data); //初始化webscoket刷新视图
