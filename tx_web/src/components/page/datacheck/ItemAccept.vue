	...
	<div class="basic" style="flex:1;display:flex=1">
		<div class="container" id="userManage">
			<el-tabs v-model="activeName" type="card" @tab-click="handleClick">
				...
				<!-- 非正常验收 -->
				<el-tab-pane label="非正常验收" name="first">
					<el-table :height="this.commonFunc.screeHeight" border class="table" ref="multipleTable" header-cell-class-name="table-header" @selection-change="handleSelectionChange"
					 :data="tables2">
						...
						<el-table-column prop="acceptTimeStamp" label="验收时间戳" align="center"></el-table-column>
						...
						<el-table-column prop="acceptProcess" label="验收工序" align="center"></el-table-column>
				...
				<!-- 正常验收 -->
				<el-tab-pane label="正常验收" name="second">
					<el-table :height="this.commonFunc.screeHeight" border class="table" ref="multipleTable" header-cell-class-name="table-header" @selection-change="handleSelectionChange"
					 :data="tables1">
						<el-table-column type="selection" width="55"></el-table-column>
						<el-table-column type="index" label="序号" align="center" width="80"></el-table-column>
	...
	export default {
		...
		methods: {
			...
			getAcceptNoticeData() {
				...
				fd.append('flag', 'getAcceptNoticeData')
				fd.append('userId', userInfo.userId)
				fd.append('level', this.level)
				this.$axios.post(that.$adminUrl + `/datacheck/ItemAccept/ItemAccept.php`, fd).then(res => {
					// console.log(res.data)
					if (res.data.code) {
						let normalAccept = 1
						let abnormalAccept = 1
						for(var i=0;i<res.data.data.length;i++){
							if(res.data.data[i].state=="正常验收"){
								res.data.data[i].index = normalAccept
								this.tableData1.push(res.data.data[i])
								normalAccept++
							}else if(res.data.data[i].state=="非正常验收"){
								res.data.data[i].index = abnormalAccept
			...
			handleJump(index, row) {
				...
				sessionStorage.setItem('itemAccepttData', JSON.stringify(row))
				// console.log(row)
				this.$router.push({
					name: 'acceptDetail',
					params: row
