<template>
	<view class="zhilin-picker">
		<uni-popup ref="popup" type="bottom" @change="popupChange">
			...
			<view class="usb">
				<uni-search-bar v-if="showSearch && options.length" :radius="100" cancelButton="none" v-model="searchVal"></uni-search-bar>
			</view>
			<view class="main" v-if="list.length">
				<scroll-view scroll-y="true" @scrolltolower="lower" :lower-threshold="1">
					...
					<view v-if="showBottom" class="isBottom" style="width: 94%;">已经到底啦~</view>
				</scroll-view>
		...
		methods: {
			...
			dataInit() {
				if (typeof this.options[0] == "string") {
					this.list = this.options.map((v, i) => ({
						label: v,
						value: v
					}))
				} else {
			...
			inputSearch() {
				...
				this.showBottom = false
				if (typeof this.options[0] == 'string') {
					this.list = this.options.filter(v => v.indexOf(val) > -1).map((v, i) => ({
						label: v,
						value: v
					}))
				} else {
					this.list = this.options.filter(v => v.label.indexOf(val) > -1)
				}
			}
		}
	}
