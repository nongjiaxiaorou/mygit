		...
		<view class="select-box" :class="activeClass ? 'select-box-show' : ''">
			...
			<view class="body-warp">
				<scroll-view class="body" scroll-y="true">
					...
					<view class="select-item" :class="[item.disabled ? 'disabled' : '',selectedArr[index] ? 'selected' : '']" 
					v-for="(item,index) in data"
					:key="item[valueName]" @tap="onSelected(index)">
			...
			setItemActiveState() {
				if (this.data.length && this.defaultSelected.length) {
					for(let i=0;i<this.data.length;i++){
						this.selectedArr.splice(i,1,false)
					}
					this.data.forEach((item, i) => {
						for (let n = 0; n < this.defaultSelected.length; n++) {
							if (!item.disabled &&item[this.valueName] === this.defaultSelected[n].value) {
