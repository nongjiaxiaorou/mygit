<template>
	<view :class="{ 'uni-card--full': isFull === true || isFull === 'true', 'uni-card--shadow': isShadow === true || isShadow === 'true' }"
	 class="uni-card" >
		...
		<!-- 标题 -->
		<view v-if="mode === 'basic' && title" class="uni-card__header" :class="complete ==='完工' ?  'complete': 'start' " @click="onClick"> 
			<view v-if="thumbnail" class="uni-card__header-extra-img-view">
				<image :src="thumbnail" class="uni-card__header-extra-img" />
			</view>
			<text class="uni-card__header-title-text"><span class="iconfont icon-jinggao"></span> {{ title }}</text>
