	...
	export default {
		...
		methods: {
			...
			touchstart(e) { //布点
				...
				that.pointList.forEach(item => {
					if (item.title == 'y'&&item.id==that.select_point.id) {
						...
						por = [item.left / 342, item.top / 242, that.box_X, that.box_Y, this.num]; //X坐标
						console.log(por);
			...
			getSavedPoint() {
				...
				// 创建最上面的画布
				var url_load = "../../../static/images/projectPic.jpg";
				that.myCanvas.drawImage(url_load, 0, 0, 1282, 906, 0, 0, 342, 243);
			...
			// 		var bbox = myCanvas.getBoundingClientRect();
					...
					for(var i=0;i<data.length;i++){
						var X_zb = data[i].Xcoordinate;  
						var Y_zb = data[i].Ycoordinate;
						var X_father = data[i].parXcoordinate;
						var Y_father = data[i].parYcoordinate; 
						...
						let endx = point_x+parseFloat(X_zb*this.small_width)+4
						let endy = point_y+parseFloat(Y_zb*this.small_height)-6
						console.log(endx+'+++++'+endy);
						this.myCanvas.beginPath();   
						this.myCanvas.strokeStyle ="#ff0000"; 
						this.myCanvas.arc(endx,endy,2,Math.PI*2,0,true);
						...
						this.myCanvas.fillText(num,endx,endy);   
					} 
