<template>
	<view>
		<!-- 页签 -->
		<view class="inv-h-w">
			<view :class="['inv-h', Inv == 0 ? 'inv-h-se' : '']" @click="Inv = 0">浇筑令({{this.list.length}})</view>
			<view :class="['inv-h', Inv == 1 ? 'inv-h-se' : '']" @click="Inv = 1">旁站({{this.list1.length+this.list2.length}})</view>
		</view>
		<view v-show="Inv == 0">
			<view class="example-body">
				<!-- 浇筑令卡片 -->
				<view v-for="item in list" :key="item.id" class="example-box">
					<uni-card :is-shadow="isShow" :title="item.projectName" @click="checkpour(item.id)">
						<view class="content-box-text">
							...
							<view class="uni-list-cell uni-list-card">计划浇筑方量:{{ item.pouringVolume }}</view>
							<view class="uni-list-cell uni-list-card">施工部位:{{ item.pouringPosition }}</view>
		...
		<view class="" v-show="Inv == 1">
			<view class="example-body">
				...
				<view v-for="item in list1" :key="item.id" class="example-box">
					<uni-card :is-shadow="isShow" :title="item.projectName" @click="checkside(item.id)">
						<view class="content-box-text">
							<view class="uni-list-cell uni-list-card">
								创建人:{{ item.creater }}
								<view>
				...
				<!-- 完成旁站 -->
				<view v-for="item in list2" :key="item.id" class="example-box">
					<uni-card :complete="complete" :is-shadow="isShow" :title="item.projectName" @click="checkside(item.id)">
						<view class="content-box-text">
							<view class="uni-list-cell uni-list-card">创建人:{{ item.creater }}</view>
							<view class="uni-list-cell uni-list-card">混凝土强度等级：{{ item.concreteGrade }}</view>
							<view class="uni-list-cell uni-list-card">计划浇筑方量：{{ item.pouringVolume }}</view>
							<view class="uni-list-cell uni-list-card">施工部位：{{ item.pouringPosition }}</view>
							<view class="uni-list-cell uni-list-card">浇筑日期：{{ item.pouringDate }}</view>
							<view class=" uni-list-card">旁站完成时间：{{ item.completeDate }}</view>
						</view>
					</uni-card>
				</view>
			</view>
		...
		<messagepush ref="select_person" :flag="'pour'" :person="'allPerson'" @refresh="refresh"></messagepush>
	</view>
</template>
...
export default {
	...
	methods: {
		getCard(){
			...
			let param2 = {
				flag: 'getcompleteside',
				buildSelData: buildSelData,
				isSelBuild: this.isSelBuild
			}
			let isLoading2 = true//是否需要加载动画
			this.myRequest.httpRequest (opts2, param2,isLoading2).then(res => {
				if ((res.data.status = 'success')) {
					...
					this.complete = '完工'; //卡片变灰
				}
				uni.hideLoading()//隐藏加载中转圈圈
				this.isloading2 = false//取消遮罩层
			}, error => {
				console.log(error);
			})
		},
					...
					uni.showModal({
						...
						success: function(res) {
							if (res.confirm) {
								for (var i = 0; i < length; i++) {
									uni.request({
										url: that.api + '/module_project/SpecialAdmin/pour.php',
										data: {
											flag: 'deletePour',
