...
<script>
	...
	export default {
		...
		methods: {
			...
			inspectQues(){
				let opts1 = {
					url: this.api + '/module_project/ExternalInspect/ExternalInspectDetail.php',
					method: 'POST'
				}
				let param1 = {
					flag: 'getInspectQues',
			...
			uploadModel(e){
				...
				uni.navigateTo({
					url:`ExternalUploadPicture?id=${that.formData.insTaskId}&fileCategory=${this.fileCategory}&imageList=${this.imageList}&taskTimeStamp=${that.formData.taskTimeStamp}`
				})
			},
			//返回上页
			back() {
