...
export default {
	...
	methods: {
		...
		confirm(){
			var that = this;
			console.log(that.cardTimeStamp);
			// // 循环遍历ID
			that.selectPerson = [];
			...
			var length = that.cardTimeStamp.length;
			for (var i = 0; i < length; i++) {
				let opts = {
					url: this.api+'/message_push/messageToInspect.php',
					method: 'POST'
				}
				let param = {
					flag: this.flag,
					selectPerson:selectPerson,
					cardTimeStamp:that.cardTimeStamp[i]
				}
			...
			this.$refs.popup.close();
			if(this.flag=='side'){
				that.$emit('assignside', that.cardTimeStamp[0]);
			}else{
