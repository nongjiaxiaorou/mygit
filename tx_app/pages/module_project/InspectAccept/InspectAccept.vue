					...
					<uni-card :is-shadow="true" :title="item.projectName">
						<view class="content-box-text">
							<view class="uni-list-cell uni-list-card">
								...
								<view>
									<checkbox-group><checkbox value="item.id" :checked="true" /></checkbox-group>
								</view>
							</view>
							...
							<view class="uni-list-cell uni-list-card">分包单位:{{item.subcontract}}</view>
							...
							<view class=" uni-list-card button_style" style="display:flex; justify-content: space-between;">
								<view>
									<button @click="toItemInspect(item)" type="primary" class="button_style">项目检查</button>
								</view>
								<view>
									<button @click="toItemAccept(item)" type="primary" class="button_style">项目验收</button>
								</view>
							</view>
						</view>
					</uni-card>
				...
				<view v-for="item in completeCardList" :key="item.id" class="example-box">
					<uni-card :complete="complete" :is-shadow="true" :title="item.projectName">
						<view class="content-box-text">
							...
							<view class=" uni-list-card button_style" style="display:flex; justify-content: space-between;">
								<view>
									<button @click="toItemInspect(item)" class="button_custom1" type="primary">项目检查</button>
								</view>
								<view>
									<button @click="toItemAccept(item)" class="button_custom1" type="primary">项目验收</button>
								</view>
							</view>
						</view>
					</uni-card>
	...
	export default {
		...
		methods: {
			...
			toItemInspect(item) {
				...
				uni.navigateTo({
					url:`ProInspect?currentData=${this.currentData}`+`&cardParam=${itemStr}`
				})
			},
			// 跳转到项目验收页面
			toItemAccept(item) {
				...
				let buildSelData = JSON.stringify(this.buildSelData)
				uni.navigateTo({
					url:`ItemAcceptProcedure?currentData=${this.currentData}`+`&cardParam=${itemStr}`+`&proTimeStamp=${this.proTimeStamp}`+`&buildSelData=${buildSelData}`
				})
			}
		},
	}
