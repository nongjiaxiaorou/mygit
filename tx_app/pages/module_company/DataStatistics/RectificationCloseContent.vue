				...
				<uni-card v-for="(item,index) in cardList" :key="index" :title="item.title" :isFull="true" :isShadow="true" :note="item.extra" :extra="item.rightCornerInfo" @click="clickCard(item)" v-show="item.isShow===true">
					<view class="flex main-box">
						...
						<view class="child-box1 flex" style="flex-direction:column;justify-content: center;">
							<view>整改累计数：{{item.rectificationNum}}</view>
							<view>未整改累计数：{{item.unRectificationNum}}</view>
	...
	export default {
		...
		methods:{
			...
			clickCard(item) {
				...
				uni.navigateTo({
					url:`RectificationCloseDetail?currentData=${currentData}`
				})
			},
			//获取整改闭合数据 未选择时间
			getRectificationContent() {
				let opts = {
					url: this.api + '/module_company/DataStatistics/RectificationClose.php',
					method: 'POST'
				}
				let param = {
					flag: "getRectificationContent",
					...
					if(res.data.code){
						...
						for(var i=0;i<res.data.data.length;i++){
							this.cardList.push({
								...
								unRectificationNum: res.data.data[i].unRectificationNum,
								extra: '点击卡片可查看详情',
								isShow: true
							})
						}
			...
			//获取整改闭合数据 已选择时间
			getRectificationSelDate() {
				let opts = {
					url: this.api + '/module_company/DataStatistics/RectificationClose.php',
					method: 'POST'
				}
				let param = {
					flag: "getRectificationSelDate",
					company: this.currentData.title,
