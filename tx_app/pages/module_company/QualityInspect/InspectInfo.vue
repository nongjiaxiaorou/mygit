<template>
	<view class="uni-fab-box">
		...
		<view v-show="Inv == 0">
			<view class="example-body">
				<view v-for="item in taskList" :key="item.taskTimeStamp" class="example-box">
					<uni-card :is-shadow="true" :title="'检查对象：'+item.inspectObj">
						<view class="flex">
		...
		<!-- <messagePush ref="select_person" :flag="'completeInsInfoPush'" :person="'allComPerson'" @refresh="complete"></messagePush> -->
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			complete(){
				//异步下发选中通知单进入整改中
				let opts1 = {
					url: this.api + '/module_company/QualityInspect/InspectInfo.php',
					method: 'POST'
				}
				let param1 = {
					flag: 'completeDefine',
			...
			trigger(e) {
				...
				let noticeList = []
				if (this.Inv == 0) {
					noticeList = Object.values(this.taskList)
				} else if (this.Inv == 2) {
